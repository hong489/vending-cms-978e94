...
const HCMasterFormPage = (props) => {
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("hCMasterForm").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  return (
    <div className="col-12 flex flex-column align-items-center">
      <div className="col-10">
        <h3 className="mb-0 ml-2">HCMasterForm</h3>
        <div className="col flex justify-content-end">
          <Button
            label="add"
            icon="pi pi-plus"
            onClick={() => setShowCreateDialog(true)}
            role="hCMasterForm-add-button"
          />
          <SplitButton
