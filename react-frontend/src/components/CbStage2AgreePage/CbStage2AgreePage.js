...
const CbStage2AgreePage = (props) => {
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("cbStage2Agree").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
