...
const CbStage1AgreePage = (props) => {
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("cbStage1Agree").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  return (
    <div className="col-12 flex flex-column align-items-center">
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <CbStage1AgreeEditDialogComponent
