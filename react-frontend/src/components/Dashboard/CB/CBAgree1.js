...
const CBAgree1 = (props) => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const eagerResult = await client.service("cbStage1Agree").find({
        query: {
          $limit: 100,
          _id: { $in: [_entity._id] },
          $populate: [
      ...
      <div className="card grid agreement stage1agreement">
        ...
        <div className="grid col-12 flex-row">
          ...
          <div className="col-2">
            <Checkbox
              onChange={(e) =>
                setValByKey("MngrProceed", e.checked ? true : null)
              }
              checked={_entity?.MngrProceed === true}
            />
            <label> Yes</label>
          </div>
          <div className="col-1">
            <Checkbox
              onChange={(e) =>
                setValByKey("MngrProceed", e.checked ? false : null)
              }
              checked={_entity?.MngrProceed === false}
            />
            <label> No</label>
          </div>
        </div>
...
export default connect(mapState, mapDispatch)(CBAgree1);
